...
Requirements:
    pip install flask werkzeug requests cryptography pynacl sqlalchemy python-dotenv bcrypt argon2-cffi
"""
...
class DeviceHSM:
    ...
    Uses Argon2id for maximum security.
    """
    ...
    def _get_device_id(self) -> bytes:
        """Get unique device identifier from multiple sources using Argon2id"""
        identifiers = []
        ...
        # Use Argon2id for device fingerprint - memory-hard, GPU-resistant
        device_hash = hash_secret_raw(
            secret=combined.encode(),
            salt=b'zcash-faucet-device-id-v1',
            time_cost=3,
        ...
        logger.info("Device HSM initialized with Argon2id fingerprint")
        return device_hash
    
    def _derive_master_key(self) -> bytes:
        """Derive master encryption key from device ID using Argon2id"""
        master_key = hash_secret_raw(
            secret=self.device_id,
            salt=b'zcash-faucet-master-key-v1',
            time_cost=3,
            memory_cost=65536,  # 64 MiB
            parallelism=4,
            hash_len=32,
        ...
        logger.info("Master key derived with Argon2id KDF")
        return master_key
...
def require_admin_auth(f):
    """Decorator for admin routes requiring HTTP Basic Auth"""
    @wraps(f)
    def decorated_function(*args, **kwargs):
        ...
        if not admin or not check_password_hash(admin.password_hash, auth.password):
            logger.warning(f"Failed admin login attempt: {auth.username}")
        ...
        logger.info(f"Admin authenticated: {auth.username}")
...
def generate_api_key():
    """Generate new API key (requires admin authentication)"""
    try:
        ...
        logger.info(f"API key generated by admin: {description}")
...
@app.route('/admin/list-api-keys', methods=['GET'])
@require_admin_auth
def list_api_keys():
...
@app.route('/admin/revoke-api-key/<int:key_id>', methods=['POST'])
@require_admin_auth
def revoke_api_key(key_id):
    """Revoke API key (requires admin authentication)"""
    try:
        api_key = db_session.query(APIKey).filter_by(id=key_id).first()
        ...
        logger.info(f"API key revoked by admin: {api_key.description}")
        
        return jsonify({
            'success': True,
            'message': f'API key "{api_key.description}" revoked'
        })
...
def create_admin_user(username: str, password: str):
    ...
    try:
        # Initialize app to get db_session
        init_app()
        
        # Check if user exists
        existing = db_session.query(AdminUser).filter_by(username=username).first()
        if existing:
        ...
        print(f"\nUse HTTP Basic Auth to access /admin/* routes\n")
...
Examples:
  ...
  # Create admin user for /admin routes
  python faucet.py --create-admin admin SecurePassword123!
  ...
  # Generate API key (CLI - requires admin user created first)
  python faucet.py --generate-api-key "My App"
    ...
    parser.add_argument('--create-admin', nargs=2, metavar=('USERNAME', 'PASSWORD'),
                      help='Create admin user for /admin routes')
