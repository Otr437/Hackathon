...
trait IX402Payment<TContractState> {
    // x402 Core: Payment request and processing
    ...
    // x402 Deferred: Batch settlement for agents
...
struct PaymentDetails {
    ...
    price_usd: u256,          // Price in USD cents (e.g., 100 = $1.00)
    usdc_amount: u256,        // Actual USDC paid (6 decimals)
    status: u8,               // 0=pending, 1=confirmed, 2=failed
    payment_scheme: u8,       // 0=immediate, 1=deferred
    resource: felt252,        // API endpoint or resource identifier
    client_id: felt252,       // Client/agent identifier
    timestamp: u64,
    confirmed_at: u64,
    block_number: u64,
}
...
mod X402Payment {
    ...
    const USDC_DECIMALS: u256 = 1000000; // USDC has 6 decimals
    ...
    struct Storage {
        ...
        // x402 Core State
        merchant_wallet: ContractAddress,    // Where USDC payments go (like x402 recipient)
        usdc_token: ContractAddress,         // USDC contract address on Starknet
        paused: bool,
        secret_key: felt252,                 // For deferred payment signatures
        ...
        // x402 Deferred payments (for AI agents)
        ...
        // Resource pricing (USD cents per unit)
